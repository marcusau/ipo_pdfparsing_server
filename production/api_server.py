...
logger = logging.getLogger('uat_ipo_api_server')

# Create handlers
c_handler = logging.StreamHandler()
f_handler = logging.FileHandler('uat_ipo_api_server.log')
...
def download_asa(stockcode:Union[str,int],eng_asa_url:str,chi_asa_url:str):
    ...
    if eng_kind is None:
        logging.error(f"cannot guess file type of {config.external_API.pdf+eng_asa_url} ")
        return {"status":"error", 'content':f"cannot guess file type of {config.external_API.pdf+eng_asa_url} "}
    elif chi_kind is None:
        logging.error(f"cannot guess file type of {config.external_API.pdf +chi_asa_url} ")
        ...
        logging.error(f"this is not pdf file in  {config.external_API.pdf + eng_asa_url} ")
        ...
        logging.info(f'finish stockcode: {stockcode} , eng  : asa url :{eng_asa_url} and save pdf to : {eng_pdf_path}')


        with open(chi_pdf_path, 'wb') as f:
            f.write(chi_asa_response.content)

        logging.info(f'finish stockcode: {stockcode} , chi : asa url :{chi_asa_url} and save pdf to : {chi_pdf_path},\n')
...
def locate_pages(pdf_file_path,tag='parties',lang=lang):
    ...
    if len(fitz_toc) == 0:
        ...
        if not pi_pages:
            logging.error(f'cannot find the pages of "directors and parties involved for stocks:{pdf_file_path}')
            return None,None
        else:
...
def fetch_pdf(pdf_path,tag='sponsors'):
    ...
    if  tag_pages is None:
        logging.error('cannot locate table of content of pdf, please check filetype and make sure the file is IPO prospectus')
        ...
        if  tag.lower() in ['underwriting','underwriter','underwriters'] :
            ...
            logging.info(f"underwriter info :{underwriters_results}")
            ...
            logging.debug(f'party info: {party_info},  ')
            ...
            if sponsor_info:
                ...
                logging.error(f" error on sponsor_info :{sponsor_info}")
...
def get_secfirm_list(sec_firm_api:str)->Dict:
    ...
    if resp.status_code is not 200:
        logging.error(f"error : response from securities firm list from webteam with status code :{resp.status_code}")
        return None
    else:
...
def main(code,eng_asa_url,chi_asa_url):
    ...
    if download_results['status']=='error':
        logging.error(f"download pdf error : {download_results} ")
        ...
        if sponsor_info['status']=='error' or underwriter_info['status']=='error':
            logging.error(f"error on extract sponsor : {sponsor_info},  underwriter info: {underwriter_info}")
            return sponsor_info
        else:
...
def ipo_pdf_parser(code,chi,eng,type=None):
    ...
    if results['status']=='error':
        logging.error(f" error on ipo_pdf_parser function : {results['status']}")
        return results
    else:
