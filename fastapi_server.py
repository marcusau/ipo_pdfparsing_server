...
async def fetch_pdf(pdf_path,tag='sponsors'):
   ...
   # print(f"tag:{tag}, toc page :{tag_pages}")
    if  tag_pages is None:
        ...
        ner_results={}
        for text in tqdm(process_text.split('\n')):
            tokens,ner_tags=await scan_ner(text=text)
...
async def get_secfirm_list(sec_firm_api:str)->Dict:
...
async def main(code,eng_asa_url,chi_asa_url):
    ...
    if download_results['status']=='error':
        ...
        sponsor_info=await fetch_pdf(pdf_path=eng_pdf_path, tag='sponsor')
       # print(f"sponsor info:{sponsor_info}")
        underwriter_info =await fetch_pdf(pdf_path=eng_pdf_path, tag='underwriting')
        ...
        if sponsor_info['status']=='error' or underwriter_info['status']=='error':
            ...
            secfirm_list=await  get_secfirm_list(sec_firm_api=config.external_API.sec_firm)
...
# @app.post(config.API.route.main)
# async def ipo_pdf_parser(code,chi,eng,type=None):
@app.post(config.API.route.main)
async def ipo_pdf_parser(request: Request):
    data= await request.json()

    print(data)
    # code,eng,chi=data['code'],data['eng'],data['chi']
    # type_= data.get('type',None)
    #
    # results=await main(code,  eng,chi)
    #
    # if results['status']!='success':
    #     return json.dumps(results)
    # else:
    #     result=results['content']
    #     return json.dumps({"status": "success" ,'content':result.get(type_,None)   if type_ else result})



if __name__ == "__main__":
    uvicorn.run(app, port=config.API.port, host=config.API.host, reload=False)